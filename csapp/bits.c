...
int allOddBits(int x) {
  int combination = x | (0x55 | (0x55 << 8) | (0x55 << 16) | (0x55 << 24));
...
int logicalNeg(int x) {
  int complement = x | (~x + 1);
  return (complement >> 31) + 1;
  /*
  return 1 >> (unsigned)(~x + 1);
  */
}
 ...
 *  Rating: 4
 */
 // special thanks to http://graphics.stanford.edu/~seander/bithacks.html#IntegerLog, 
 // where the algorithm below was copied.  
 ...
 *   Rating: 4
 */
unsigned floatPower2(register int x) {
  if (x >= 0)
  {
    if (x <= 0x80)
    {
